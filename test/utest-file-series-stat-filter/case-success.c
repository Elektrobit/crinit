/**
 * @file case-success.c
 * @brief Unit test for EBCL_statFilter(), successful execution.
 *
...
static void EBCL_testVariant(const char *path, int baseDirFd, bool followLinks) {
    ...
    if (followLinks) {
        print_message("Testing EBCL_statFilter with path '%s', dir file descriptor '%d', following links.\n", path,
                      baseDirFd);
    } else {
        print_message("Testing EBCL_statFilter with path '%s', dir file descriptor '%d', without following links.\n",
                      path, baseDirFd);
    }
    ...
    assert_int_not_equal(EBCL_statFilter(path, baseDirFd, followLinks), 0);
}
