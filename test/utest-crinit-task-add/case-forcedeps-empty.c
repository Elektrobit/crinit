 ...
 * @brief Unit test for EBCL_crinitTaskAdd() with forceDeps as empty string.
 *
...
void EBCL_crinitTaskAddTestForceDepsEmpty(void **state) {
    ...
    expect_string(__wrap_EBCL_buildRtimCmd, vargs[2], "@empty");
    ...
    will_return(__wrap_EBCL_destroyRtimCmd, 0);
    assert_int_equal(EBCL_crinitTaskAdd(TEST_CONFIG_FILE, false, ""), 0);
}
