...
/**
 * Structure holding a command or response message with its ebcl_RtimOp opcode and arguments array.
 */
typedef struct ebcl_RtimCmd {
    ebcl_RtimOp op;  ///< The command or response opcode (see rtimopmap.h).
    int argc;        ///< The number of arguments.
    char **args;     ///< String array of arguments.
} ebcl_RtimCmd;

/**
 * Create an ebcl_RtimCmd from an opcode and an argument list.
 *
 * Will allocate memory for the argument string array which should be freed using EBCL_destroyRtimCmd() when no longer
 * needed.
 *
 * @param c     The ebcl_RtimCmd to build.
...
/**
 * Free memory in an ebcl_RtimCmd allocated by EBCL_buildRtimCmd() or EBCL_parseRtimCmd().
 *
 * Will free the memory for the argument array in the structure.
 *
 * @param c  The ebcl_RtimCmd from which the memory should be freed.
 *
 * @return 0 on success, -1 otherwise
 */
...
/**
 * Parses a string into an ebcl_RtimCmd.
 *
 ...
 * EBCL_destroyRtimCmd().
 *
 * @param out     The ebcl_RtimCmd to create.
...
/**
 * Generates a string representation of an ebcl_RtimCmd.
 *
 ...
 * @param cmd     The ebcl_RtimCmd to generate the string from.
 *
 * @return 0 on success, -1 otherwise
 */
int EBCL_rtimCmdToMsgStr(char **out, size_t *outLen, const ebcl_RtimCmd *cmd);
/**
 * Executes an ebcl_RtimCmd if it contains a valid command.
 *
 * For the implementations of the different possible commands see rtimcmd.c.
 *
 * @param ctx  Pointer to the ebcl_TaskDB the command shall be executed on.
